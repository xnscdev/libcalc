/*************************************************************************
 * number-mul.c -- This file is part of libcalc.                         *
...
DEFINE_TEST (n_si)
{
  CalcNumber *a = calc_number_new_ui (TEST_VALUE_A);
  CalcNumber *b = NULL;
  calc_number_mul_si (&b, a, -TEST_VALUE_B);
  g_object_unref (a);
  if (b == NULL)
    abort ();
  assert_num_equals_si (b, TEST_VALUE_A * -TEST_VALUE_B);
  g_object_unref (b);
}
